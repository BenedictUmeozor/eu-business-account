...
const data = [
  { name: "Total money in", value: 875.0, color: Colors.positive },
  { name: "Total money out", value: 410.25, color: Colors.pending },
];
          ...
          {data.map((entry, index) => (
            <div key={index} className="flex items-start gap-2">
              ...
              <div className="space-y-0.5">
                ...
                <p className="font-medium">kr.{entry.value.toFixed(2)}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="p-6 shadow rounded-md flex flex-col gap-6 justify-between bg-white">
        <header className="flex items-start justify-between">
          <div className="space-y-2">
            <div className="h-11 w-11 rounded-full overflow-hidden grid place-items-center bg-red-50">
              <img
                src={ENDPOINTS.FLAG_URL("dk")}
                alt="dk"
                className="w-full h-full object-cover"
              />
            </div>
            <p className="font-medium text-grey-500">Total DKK Balance</p>
        ...
        <div className="flex items-end justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link
              to="/dashboard/send-money?currency=DKK"
              state={{ currency: "DKK" }}
...
const ClipboardCopy = () => {
  ...
  return (
    <div className="flex items-center gap-0.5 bg-gray-50 rounded-md p-1 px-2.5">
      <span className="text-sm text-grey-500">Hellome... 7755203366</span>
      <CopyToClipboard text="Hellome... 7755203366" onCopy={handleCopy}>
        <Button
          type="text"
          icon={
            copied ? (
