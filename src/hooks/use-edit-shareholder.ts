...
const useEditShareholder = ({ onSuccess, onError }: EditShareholderParams = {}) => {
  ...
  const editShareholder = async (formData: any, frontImage: File | null, backImage: File | null, documentType: string) => {
    try {
      ...
      if (frontImage) {
        ...
        frontFormData.append("shareholder_token", response.shareholder_token);
      ...
      if (backImage) {
        ...
        backFormData.append("shareholder_token", response.shareholder_token);
